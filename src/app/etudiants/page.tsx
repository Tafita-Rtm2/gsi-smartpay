...
export default function EtudiantsPage() {
  ...
  // Payment form — same as paiements page
  ...
  const openPayment = (s: DBStudent, ec: DBEcolage | undefined) => {
  ...
  const handleSavePayment = async () => {
    if (!ecolageStudent || !payForm.montant) return;
    setSaving(true);
    ...
    // Also create a paiement record
    try {
      const { createPaiement } = await import("@/lib/api");
      await createPaiement({
        etudiantId:  getStudentId(ecolageStudent),
        etudiantNom: getStudentName(ecolageStudent),
        matricule:   ecolageStudent.matricule,
        campus:      ecolageStudent.campus || "",
        filiere:     ecolageStudent.filiere || "",
        classe:      ecolageStudent.niveau || "L1",
        montant,
        date:        payForm.date,
  ...
  const Avatar = ({ s, size = 9 }: { s: DBStudent; size?: number }) => {
    ...
    return (
      <div
        style={{ background: etabColor, width: px, height: px, minWidth: px }}
        className="rounded-full overflow-hidden shrink-0 flex items-center justify-center text-white font-bold text-xs border-2 border-white shadow-sm"
      >
        {s.photo
  ...
  return (
    <div className="space-y-5">
        ...
        {STAT_TABS.map(({ id, label, count, color, border, icon: Icon }) => (
          <button key={id} onClick={() => setStatutTab(id)}
            className={clsx("card text-left transition-all border-2",
              statutTab === id ? `${border} shadow-md` : "border-transparent hover:border-slate-200")}>
      ...
      <div className="card p-0 overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-slate-400 space-y-3">
            ...
            <button onClick={() => { setSearch(""); setFiliereFilter("Toutes"); setStatutTab("tous"); }}
              className="text-xs text-brand-600 hover:underline">Reinitialiser les filtres</button>
          </div>
        ) : (
          <>
            {/* Desktop */}
                  ...
                  {filtered.map(s => {
                    ...
                    return (
                      <tr key={getStudentId(s)} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {s.niveau && (
                            <span className="bg-brand-50 text-brand-700 text-xs font-bold px-2 py-0.5 rounded-full">{s.niveau}</span>
                          )}
                        </td>
                          ...
                          {ec && ec.montantDu > 0 ? (
                            <div className="text-xs space-y-0.5">
                              ...
                              {ec.montantDu > ec.montantPaye && (
                                <div className="text-red-500 font-medium">Reste: {formatMGA(ec.montantDu - ec.montantPaye)}</div>
                              )}
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            <button onClick={() => openPayment(s, ec)} title="Enregistrer paiement"
                ...
                return (
                  <div key={getStudentId(s)} className="p-4 space-y-2">
                    ...
                    <div className="flex gap-2 flex-wrap pt-1">
                      ...
                      <button onClick={() => openPayment(s, ec)}
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
              ...
              <div className="pt-10 px-6 pb-6 space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => { openPayment(profileStudent, ec); setProfileStudent(null); }}
      ...
      {/* ─── PAYMENT MODAL (identique a paiements page) ─── */}
      {ecolageStudent && (
          ...
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <div className="flex items-center gap-3 bg-slate-50 border border-slate-100 rounded-xl p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-400">{ecolageStudent.matricule} · {ecolageStudent.filiere}</div>
                {editEcolageData && editEcolageData.montantDu > 0 ? (
                  <div className="text-xs mt-0.5 text-amber-600 font-medium">
                    Reste: {formatMGA(editEcolageData.montantDu - editEcolageData.montantPaye)}
                  </div>
                ) : (
                  <div className="text-xs mt-0.5 text-slate-400">Aucun ecolage defini</div>
                )}
              </div>
            </div>
            ...
            {/* Mode — Especes only, shown as badge */}
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl px-3 py-2 text-xs text-blue-700 flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-blue-200 flex items-center justify-center text-blue-700 font-bold text-xs shrink-0">
                {(currentUser?.prenom || "A")[0]}
              </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSavePayment}
                disabled={saving || !payForm.montant}
